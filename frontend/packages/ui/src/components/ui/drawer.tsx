...
>(({ className, ...props }, ref) => (
  <DrawerPrimitive.Overlay
    ref={ref}
    className={cn('fixed inset-0 z-50 bg-dark-900/50', className)}
    {...props}
  />
))
...
>(({ className, children, showOverlay = true, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ref={ref}
      className={cn(
        'fixed inset-x-0 bottom-0 z-50 flex h-full max-h-[96dvh] flex-col rounded-t-2xl bg-white font-nunito-sans shadow-xl outline-none',
        className,
      )}
      {...props}
    >
