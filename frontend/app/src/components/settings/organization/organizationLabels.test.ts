...
describe('memberSubtitle', () => {
  it('sums the whole subtree', () => {
    ...
    expect(memberSubtitle(node)).toBe('9 Personen')
  })

  it('uses the singular for a single person', () => {
    expect(memberSubtitle(treeOf([org('amt', undefined, 'Grünflächenamt', 1)]))).toBe('1 Person')
  })
  ...
  it('returns null for an empty subtree, so the row stays single-line', () => {
    ...
    expect(memberSubtitle(node)).toBeNull()
  })
})
...
describe('memberSummary', () => {
  it('names the sub-organizations the people are spread over', () => {
    ...
    expect(memberSummary(node)).toBe('9 Personen in 2 Unterorganisationen')
  })

  it('leaves out the sub-organizations when there are none', () => {
    expect(memberSummary(treeOf([org('amt', undefined, 'Grünflächenamt', 3)]))).toBe('3 Personen')
  })
  ...
  it('says nobody is assigned instead of counting empty sub-organizations', () => {
    ...
    expect(memberSummary(node)).toBe('Keine Personen zugeordnet')
  })
})

describe('directChildrenLine', () => {
  it('uses the singular for one sub-organization', () => {
    expect(directChildrenLine(1)).toBe('1 direkte Unterorganisation')
  })

  it('uses the plural for several', () => {
    expect(directChildrenLine(3)).toBe('3 direkte Unterorganisationen')
  })
})
