...
func NewPluginManager(authRepo auth.AuthRepository, opts ...PluginManagerOption) *PluginManager {
...
func (p *PluginManager) Register(ctx context.Context, plugin *plugin.Plugin) (*auth.ClientToken, error) {
	...
	if err != nil {
		...
		return nil, ports.MapError(ctx, errors.Join(err, errors.New("failed to login plugin with credantials")), ports.ErrorLogAll)
	}
	...
	if _, ok := p.plugins[plugin.Slug]; ok {
		...
		return nil, ports.ErrPluginRegistered
	}
...
func (p *PluginManager) RefreshToken(ctx context.Context, auth *plugin.AuthPlugin, slug string) (*auth.ClientToken, error) {
	log := logger.GetLogger(ctx)
	...
	if err != nil {
		...
		return nil, ports.MapError(ctx, errors.Join(err, errors.New("failed to refresh plugin token with credantials")), ports.ErrorLogAll)
	}

	return token, err
}

func (p *PluginManager) Get(ctx context.Context, slug string) (plugin.Plugin, error) {
	log := logger.GetLogger(ctx)
	p.mutex.RLock()
	...
	if !ok {
		...
		return plugin, ports.ErrPluginNotRegistered
	}

	return plugin, nil
}

func (p *PluginManager) GetAll(_ context.Context) ([]plugin.Plugin, []time.Time) {
	p.mutex.RLock()
	defer p.mutex.RUnlock()
...
func (p *PluginManager) HeartBeat(ctx context.Context, slug string) error {
	...
	if slug == "" {
		...
		return ports.NewError(ports.BadRequest, "slug is empty")
	}
	...
	if _, ok := p.heartbeats[slug]; !ok {
		log.Error("the plugin is not registered in the system", "plugin_slug", slug)
		return ports.ErrPluginNotRegistered
	}
