...
func (r *KeycloakRepository) RefreshToken(ctx context.Context, refreshToken string) (*auth.ClientToken, error) {
	log := logger.GetLogger(ctx)
	client := gocloak.NewClient(r.cfg.OidcProvider.BaseURL)
...
func (r *KeycloakRepository) GetAccessTokenFromClientCredentials(ctx context.Context, clientID, clientSecret string) (*auth.ClientToken, error) {
	log := logger.GetLogger(ctx)
	client := gocloak.NewClient(r.cfg.OidcProvider.BaseURL)
