...
func TestKeyCloakUserRepo_GetAllByRole(t *testing.T) {
	t.Run("should get all users by role successfully", func(t *testing.T) {
		...
		user3 := &shared.User{
			...
			EmployeeID:  "EMP001",
			PhoneNumber: "+49 123456789",
			Roles:       []shared.UserRole{shared.UserRoleSmarteGrenzregion},
		}
		user4 := &shared.User{
			Username:    "user4",
			FirstName:   "Jane",
		...
		// when
		users, err := userRepo.GetAllByRole(context.Background(), shared.UserRoleGreenEcolution)
...
func containsUser(userList []*shared.User, targetUser shared.User) bool {
