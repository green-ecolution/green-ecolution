// Code generated by mockery v2.52.2. DO NOT EDIT.

package shared

import (
	context "context"

	shared "github.com/green-ecolution/green-ecolution/backend/internal/domain/shared"
	mock "github.com/stretchr/testify/mock"
)

// MockTreeClusterRepository is an autogenerated mock type for the TreeClusterRepository type
type MockTreeClusterRepository struct {
	mock.Mock
}

type MockTreeClusterRepository_Expecter struct {
	mock *mock.Mock
}

func (_m *MockTreeClusterRepository) EXPECT() *MockTreeClusterRepository_Expecter {
	return &MockTreeClusterRepository_Expecter{mock: &_m.Mock}
}

// Archive provides a mock function with given fields: ctx, id
func (_m *MockTreeClusterRepository) Archive(ctx context.Context, id int32) error {
	ret := _m.Called(ctx, id)

	if len(ret) == 0 {
		panic("no return value specified for Archive")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, int32) error); ok {
		r0 = rf(ctx, id)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// MockTreeClusterRepository_Archive_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Archive'
type MockTreeClusterRepository_Archive_Call struct {
	*mock.Call
}

// Archive is a helper method to define mock.On call
//   - ctx context.Context
//   - id int32
func (_e *MockTreeClusterRepository_Expecter) Archive(ctx interface{}, id interface{}) *MockTreeClusterRepository_Archive_Call {
	return &MockTreeClusterRepository_Archive_Call{Call: _e.mock.On("Archive", ctx, id)}
}

func (_c *MockTreeClusterRepository_Archive_Call) Run(run func(ctx context.Context, id int32)) *MockTreeClusterRepository_Archive_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(int32))
	})
	return _c
}

func (_c *MockTreeClusterRepository_Archive_Call) Return(_a0 error) *MockTreeClusterRepository_Archive_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockTreeClusterRepository_Archive_Call) RunAndReturn(run func(context.Context, int32) error) *MockTreeClusterRepository_Archive_Call {
	_c.Call.Return(run)
	return _c
}

// Create provides a mock function with given fields: ctx, fn
func (_m *MockTreeClusterRepository) Create(ctx context.Context, fn func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)) (*shared.TreeCluster, error) {
	ret := _m.Called(ctx, fn)

	if len(ret) == 0 {
		panic("no return value specified for Create")
	}

	var r0 *shared.TreeCluster
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)) (*shared.TreeCluster, error)); ok {
		return rf(ctx, fn)
	}
	if rf, ok := ret.Get(0).(func(context.Context, func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)) *shared.TreeCluster); ok {
		r0 = rf(ctx, fn)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*shared.TreeCluster)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)) error); ok {
		r1 = rf(ctx, fn)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// MockTreeClusterRepository_Create_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Create'
type MockTreeClusterRepository_Create_Call struct {
	*mock.Call
}

// Create is a helper method to define mock.On call
//   - ctx context.Context
//   - fn func(*shared.TreeCluster , shared.TreeClusterRepository)(bool , error)
func (_e *MockTreeClusterRepository_Expecter) Create(ctx interface{}, fn interface{}) *MockTreeClusterRepository_Create_Call {
	return &MockTreeClusterRepository_Create_Call{Call: _e.mock.On("Create", ctx, fn)}
}

func (_c *MockTreeClusterRepository_Create_Call) Run(run func(ctx context.Context, fn func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error))) *MockTreeClusterRepository_Create_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)))
	})
	return _c
}

func (_c *MockTreeClusterRepository_Create_Call) Return(_a0 *shared.TreeCluster, _a1 error) *MockTreeClusterRepository_Create_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *MockTreeClusterRepository_Create_Call) RunAndReturn(run func(context.Context, func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)) (*shared.TreeCluster, error)) *MockTreeClusterRepository_Create_Call {
	_c.Call.Return(run)
	return _c
}

// Delete provides a mock function with given fields: ctx, id
func (_m *MockTreeClusterRepository) Delete(ctx context.Context, id int32) error {
	ret := _m.Called(ctx, id)

	if len(ret) == 0 {
		panic("no return value specified for Delete")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, int32) error); ok {
		r0 = rf(ctx, id)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// MockTreeClusterRepository_Delete_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Delete'
type MockTreeClusterRepository_Delete_Call struct {
	*mock.Call
}

// Delete is a helper method to define mock.On call
//   - ctx context.Context
//   - id int32
func (_e *MockTreeClusterRepository_Expecter) Delete(ctx interface{}, id interface{}) *MockTreeClusterRepository_Delete_Call {
	return &MockTreeClusterRepository_Delete_Call{Call: _e.mock.On("Delete", ctx, id)}
}

func (_c *MockTreeClusterRepository_Delete_Call) Run(run func(ctx context.Context, id int32)) *MockTreeClusterRepository_Delete_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(int32))
	})
	return _c
}

func (_c *MockTreeClusterRepository_Delete_Call) Return(_a0 error) *MockTreeClusterRepository_Delete_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockTreeClusterRepository_Delete_Call) RunAndReturn(run func(context.Context, int32) error) *MockTreeClusterRepository_Delete_Call {
	_c.Call.Return(run)
	return _c
}

// GetAll provides a mock function with given fields: ctx, query
func (_m *MockTreeClusterRepository) GetAll(ctx context.Context, query shared.TreeClusterQuery) ([]*shared.TreeCluster, int64, error) {
	ret := _m.Called(ctx, query)

	if len(ret) == 0 {
		panic("no return value specified for GetAll")
	}

	var r0 []*shared.TreeCluster
	var r1 int64
	var r2 error
	if rf, ok := ret.Get(0).(func(context.Context, shared.TreeClusterQuery) ([]*shared.TreeCluster, int64, error)); ok {
		return rf(ctx, query)
	}
	if rf, ok := ret.Get(0).(func(context.Context, shared.TreeClusterQuery) []*shared.TreeCluster); ok {
		r0 = rf(ctx, query)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*shared.TreeCluster)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, shared.TreeClusterQuery) int64); ok {
		r1 = rf(ctx, query)
	} else {
		r1 = ret.Get(1).(int64)
	}

	if rf, ok := ret.Get(2).(func(context.Context, shared.TreeClusterQuery) error); ok {
		r2 = rf(ctx, query)
	} else {
		r2 = ret.Error(2)
	}

	return r0, r1, r2
}

// MockTreeClusterRepository_GetAll_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetAll'
type MockTreeClusterRepository_GetAll_Call struct {
	*mock.Call
}

// GetAll is a helper method to define mock.On call
//   - ctx context.Context
//   - query shared.TreeClusterQuery
func (_e *MockTreeClusterRepository_Expecter) GetAll(ctx interface{}, query interface{}) *MockTreeClusterRepository_GetAll_Call {
	return &MockTreeClusterRepository_GetAll_Call{Call: _e.mock.On("GetAll", ctx, query)}
}

func (_c *MockTreeClusterRepository_GetAll_Call) Run(run func(ctx context.Context, query shared.TreeClusterQuery)) *MockTreeClusterRepository_GetAll_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(shared.TreeClusterQuery))
	})
	return _c
}

func (_c *MockTreeClusterRepository_GetAll_Call) Return(_a0 []*shared.TreeCluster, _a1 int64, _a2 error) *MockTreeClusterRepository_GetAll_Call {
	_c.Call.Return(_a0, _a1, _a2)
	return _c
}

func (_c *MockTreeClusterRepository_GetAll_Call) RunAndReturn(run func(context.Context, shared.TreeClusterQuery) ([]*shared.TreeCluster, int64, error)) *MockTreeClusterRepository_GetAll_Call {
	_c.Call.Return(run)
	return _c
}

// GetAllLatestSensorDataByClusterID provides a mock function with given fields: ctx, tcID
func (_m *MockTreeClusterRepository) GetAllLatestSensorDataByClusterID(ctx context.Context, tcID int32) ([]*shared.SensorData, error) {
	ret := _m.Called(ctx, tcID)

	if len(ret) == 0 {
		panic("no return value specified for GetAllLatestSensorDataByClusterID")
	}

	var r0 []*shared.SensorData
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, int32) ([]*shared.SensorData, error)); ok {
		return rf(ctx, tcID)
	}
	if rf, ok := ret.Get(0).(func(context.Context, int32) []*shared.SensorData); ok {
		r0 = rf(ctx, tcID)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*shared.SensorData)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, int32) error); ok {
		r1 = rf(ctx, tcID)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// MockTreeClusterRepository_GetAllLatestSensorDataByClusterID_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetAllLatestSensorDataByClusterID'
type MockTreeClusterRepository_GetAllLatestSensorDataByClusterID_Call struct {
	*mock.Call
}

// GetAllLatestSensorDataByClusterID is a helper method to define mock.On call
//   - ctx context.Context
//   - tcID int32
func (_e *MockTreeClusterRepository_Expecter) GetAllLatestSensorDataByClusterID(ctx interface{}, tcID interface{}) *MockTreeClusterRepository_GetAllLatestSensorDataByClusterID_Call {
	return &MockTreeClusterRepository_GetAllLatestSensorDataByClusterID_Call{Call: _e.mock.On("GetAllLatestSensorDataByClusterID", ctx, tcID)}
}

func (_c *MockTreeClusterRepository_GetAllLatestSensorDataByClusterID_Call) Run(run func(ctx context.Context, tcID int32)) *MockTreeClusterRepository_GetAllLatestSensorDataByClusterID_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(int32))
	})
	return _c
}

func (_c *MockTreeClusterRepository_GetAllLatestSensorDataByClusterID_Call) Return(_a0 []*shared.SensorData, _a1 error) *MockTreeClusterRepository_GetAllLatestSensorDataByClusterID_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *MockTreeClusterRepository_GetAllLatestSensorDataByClusterID_Call) RunAndReturn(run func(context.Context, int32) ([]*shared.SensorData, error)) *MockTreeClusterRepository_GetAllLatestSensorDataByClusterID_Call {
	_c.Call.Return(run)
	return _c
}

// GetAllRegionsWithWateringPlanCount provides a mock function with given fields: ctx
func (_m *MockTreeClusterRepository) GetAllRegionsWithWateringPlanCount(ctx context.Context) ([]*shared.RegionEvaluation, error) {
	ret := _m.Called(ctx)

	if len(ret) == 0 {
		panic("no return value specified for GetAllRegionsWithWateringPlanCount")
	}

	var r0 []*shared.RegionEvaluation
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context) ([]*shared.RegionEvaluation, error)); ok {
		return rf(ctx)
	}
	if rf, ok := ret.Get(0).(func(context.Context) []*shared.RegionEvaluation); ok {
		r0 = rf(ctx)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*shared.RegionEvaluation)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context) error); ok {
		r1 = rf(ctx)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// MockTreeClusterRepository_GetAllRegionsWithWateringPlanCount_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetAllRegionsWithWateringPlanCount'
type MockTreeClusterRepository_GetAllRegionsWithWateringPlanCount_Call struct {
	*mock.Call
}

// GetAllRegionsWithWateringPlanCount is a helper method to define mock.On call
//   - ctx context.Context
func (_e *MockTreeClusterRepository_Expecter) GetAllRegionsWithWateringPlanCount(ctx interface{}) *MockTreeClusterRepository_GetAllRegionsWithWateringPlanCount_Call {
	return &MockTreeClusterRepository_GetAllRegionsWithWateringPlanCount_Call{Call: _e.mock.On("GetAllRegionsWithWateringPlanCount", ctx)}
}

func (_c *MockTreeClusterRepository_GetAllRegionsWithWateringPlanCount_Call) Run(run func(ctx context.Context)) *MockTreeClusterRepository_GetAllRegionsWithWateringPlanCount_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context))
	})
	return _c
}

func (_c *MockTreeClusterRepository_GetAllRegionsWithWateringPlanCount_Call) Return(_a0 []*shared.RegionEvaluation, _a1 error) *MockTreeClusterRepository_GetAllRegionsWithWateringPlanCount_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *MockTreeClusterRepository_GetAllRegionsWithWateringPlanCount_Call) RunAndReturn(run func(context.Context) ([]*shared.RegionEvaluation, error)) *MockTreeClusterRepository_GetAllRegionsWithWateringPlanCount_Call {
	_c.Call.Return(run)
	return _c
}

// GetByID provides a mock function with given fields: ctx, id
func (_m *MockTreeClusterRepository) GetByID(ctx context.Context, id int32) (*shared.TreeCluster, error) {
	ret := _m.Called(ctx, id)

	if len(ret) == 0 {
		panic("no return value specified for GetByID")
	}

	var r0 *shared.TreeCluster
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, int32) (*shared.TreeCluster, error)); ok {
		return rf(ctx, id)
	}
	if rf, ok := ret.Get(0).(func(context.Context, int32) *shared.TreeCluster); ok {
		r0 = rf(ctx, id)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*shared.TreeCluster)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, int32) error); ok {
		r1 = rf(ctx, id)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// MockTreeClusterRepository_GetByID_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetByID'
type MockTreeClusterRepository_GetByID_Call struct {
	*mock.Call
}

// GetByID is a helper method to define mock.On call
//   - ctx context.Context
//   - id int32
func (_e *MockTreeClusterRepository_Expecter) GetByID(ctx interface{}, id interface{}) *MockTreeClusterRepository_GetByID_Call {
	return &MockTreeClusterRepository_GetByID_Call{Call: _e.mock.On("GetByID", ctx, id)}
}

func (_c *MockTreeClusterRepository_GetByID_Call) Run(run func(ctx context.Context, id int32)) *MockTreeClusterRepository_GetByID_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(int32))
	})
	return _c
}

func (_c *MockTreeClusterRepository_GetByID_Call) Return(_a0 *shared.TreeCluster, _a1 error) *MockTreeClusterRepository_GetByID_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *MockTreeClusterRepository_GetByID_Call) RunAndReturn(run func(context.Context, int32) (*shared.TreeCluster, error)) *MockTreeClusterRepository_GetByID_Call {
	_c.Call.Return(run)
	return _c
}

// GetByIDs provides a mock function with given fields: ctx, ids
func (_m *MockTreeClusterRepository) GetByIDs(ctx context.Context, ids []int32) ([]*shared.TreeCluster, error) {
	ret := _m.Called(ctx, ids)

	if len(ret) == 0 {
		panic("no return value specified for GetByIDs")
	}

	var r0 []*shared.TreeCluster
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, []int32) ([]*shared.TreeCluster, error)); ok {
		return rf(ctx, ids)
	}
	if rf, ok := ret.Get(0).(func(context.Context, []int32) []*shared.TreeCluster); ok {
		r0 = rf(ctx, ids)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*shared.TreeCluster)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, []int32) error); ok {
		r1 = rf(ctx, ids)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// MockTreeClusterRepository_GetByIDs_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetByIDs'
type MockTreeClusterRepository_GetByIDs_Call struct {
	*mock.Call
}

// GetByIDs is a helper method to define mock.On call
//   - ctx context.Context
//   - ids []int32
func (_e *MockTreeClusterRepository_Expecter) GetByIDs(ctx interface{}, ids interface{}) *MockTreeClusterRepository_GetByIDs_Call {
	return &MockTreeClusterRepository_GetByIDs_Call{Call: _e.mock.On("GetByIDs", ctx, ids)}
}

func (_c *MockTreeClusterRepository_GetByIDs_Call) Run(run func(ctx context.Context, ids []int32)) *MockTreeClusterRepository_GetByIDs_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].([]int32))
	})
	return _c
}

func (_c *MockTreeClusterRepository_GetByIDs_Call) Return(_a0 []*shared.TreeCluster, _a1 error) *MockTreeClusterRepository_GetByIDs_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *MockTreeClusterRepository_GetByIDs_Call) RunAndReturn(run func(context.Context, []int32) ([]*shared.TreeCluster, error)) *MockTreeClusterRepository_GetByIDs_Call {
	_c.Call.Return(run)
	return _c
}

// GetCenterPoint provides a mock function with given fields: ctx, id
func (_m *MockTreeClusterRepository) GetCenterPoint(ctx context.Context, id int32) (*shared.Coordinate, error) {
	ret := _m.Called(ctx, id)

	if len(ret) == 0 {
		panic("no return value specified for GetCenterPoint")
	}

	var r0 *shared.Coordinate
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, int32) (*shared.Coordinate, error)); ok {
		return rf(ctx, id)
	}
	if rf, ok := ret.Get(0).(func(context.Context, int32) *shared.Coordinate); ok {
		r0 = rf(ctx, id)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*shared.Coordinate)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, int32) error); ok {
		r1 = rf(ctx, id)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// MockTreeClusterRepository_GetCenterPoint_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetCenterPoint'
type MockTreeClusterRepository_GetCenterPoint_Call struct {
	*mock.Call
}

// GetCenterPoint is a helper method to define mock.On call
//   - ctx context.Context
//   - id int32
func (_e *MockTreeClusterRepository_Expecter) GetCenterPoint(ctx interface{}, id interface{}) *MockTreeClusterRepository_GetCenterPoint_Call {
	return &MockTreeClusterRepository_GetCenterPoint_Call{Call: _e.mock.On("GetCenterPoint", ctx, id)}
}

func (_c *MockTreeClusterRepository_GetCenterPoint_Call) Run(run func(ctx context.Context, id int32)) *MockTreeClusterRepository_GetCenterPoint_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(int32))
	})
	return _c
}

func (_c *MockTreeClusterRepository_GetCenterPoint_Call) Return(_a0 *shared.Coordinate, _a1 error) *MockTreeClusterRepository_GetCenterPoint_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *MockTreeClusterRepository_GetCenterPoint_Call) RunAndReturn(run func(context.Context, int32) (*shared.Coordinate, error)) *MockTreeClusterRepository_GetCenterPoint_Call {
	_c.Call.Return(run)
	return _c
}

// GetCount provides a mock function with given fields: ctx, query
func (_m *MockTreeClusterRepository) GetCount(ctx context.Context, query shared.TreeClusterQuery) (int64, error) {
	ret := _m.Called(ctx, query)

	if len(ret) == 0 {
		panic("no return value specified for GetCount")
	}

	var r0 int64
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, shared.TreeClusterQuery) (int64, error)); ok {
		return rf(ctx, query)
	}
	if rf, ok := ret.Get(0).(func(context.Context, shared.TreeClusterQuery) int64); ok {
		r0 = rf(ctx, query)
	} else {
		r0 = ret.Get(0).(int64)
	}

	if rf, ok := ret.Get(1).(func(context.Context, shared.TreeClusterQuery) error); ok {
		r1 = rf(ctx, query)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// MockTreeClusterRepository_GetCount_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetCount'
type MockTreeClusterRepository_GetCount_Call struct {
	*mock.Call
}

// GetCount is a helper method to define mock.On call
//   - ctx context.Context
//   - query shared.TreeClusterQuery
func (_e *MockTreeClusterRepository_Expecter) GetCount(ctx interface{}, query interface{}) *MockTreeClusterRepository_GetCount_Call {
	return &MockTreeClusterRepository_GetCount_Call{Call: _e.mock.On("GetCount", ctx, query)}
}

func (_c *MockTreeClusterRepository_GetCount_Call) Run(run func(ctx context.Context, query shared.TreeClusterQuery)) *MockTreeClusterRepository_GetCount_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(shared.TreeClusterQuery))
	})
	return _c
}

func (_c *MockTreeClusterRepository_GetCount_Call) Return(_a0 int64, _a1 error) *MockTreeClusterRepository_GetCount_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *MockTreeClusterRepository_GetCount_Call) RunAndReturn(run func(context.Context, shared.TreeClusterQuery) (int64, error)) *MockTreeClusterRepository_GetCount_Call {
	_c.Call.Return(run)
	return _c
}

// LinkTreesToCluster provides a mock function with given fields: ctx, treeClusterID, treeIDs
func (_m *MockTreeClusterRepository) LinkTreesToCluster(ctx context.Context, treeClusterID int32, treeIDs []int32) error {
	ret := _m.Called(ctx, treeClusterID, treeIDs)

	if len(ret) == 0 {
		panic("no return value specified for LinkTreesToCluster")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, int32, []int32) error); ok {
		r0 = rf(ctx, treeClusterID, treeIDs)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// MockTreeClusterRepository_LinkTreesToCluster_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'LinkTreesToCluster'
type MockTreeClusterRepository_LinkTreesToCluster_Call struct {
	*mock.Call
}

// LinkTreesToCluster is a helper method to define mock.On call
//   - ctx context.Context
//   - treeClusterID int32
//   - treeIDs []int32
func (_e *MockTreeClusterRepository_Expecter) LinkTreesToCluster(ctx interface{}, treeClusterID interface{}, treeIDs interface{}) *MockTreeClusterRepository_LinkTreesToCluster_Call {
	return &MockTreeClusterRepository_LinkTreesToCluster_Call{Call: _e.mock.On("LinkTreesToCluster", ctx, treeClusterID, treeIDs)}
}

func (_c *MockTreeClusterRepository_LinkTreesToCluster_Call) Run(run func(ctx context.Context, treeClusterID int32, treeIDs []int32)) *MockTreeClusterRepository_LinkTreesToCluster_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(int32), args[2].([]int32))
	})
	return _c
}

func (_c *MockTreeClusterRepository_LinkTreesToCluster_Call) Return(_a0 error) *MockTreeClusterRepository_LinkTreesToCluster_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockTreeClusterRepository_LinkTreesToCluster_Call) RunAndReturn(run func(context.Context, int32, []int32) error) *MockTreeClusterRepository_LinkTreesToCluster_Call {
	_c.Call.Return(run)
	return _c
}

// Update provides a mock function with given fields: ctx, id, fn
func (_m *MockTreeClusterRepository) Update(ctx context.Context, id int32, fn func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)) error {
	ret := _m.Called(ctx, id, fn)

	if len(ret) == 0 {
		panic("no return value specified for Update")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, int32, func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)) error); ok {
		r0 = rf(ctx, id, fn)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// MockTreeClusterRepository_Update_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Update'
type MockTreeClusterRepository_Update_Call struct {
	*mock.Call
}

// Update is a helper method to define mock.On call
//   - ctx context.Context
//   - id int32
//   - fn func(*shared.TreeCluster , shared.TreeClusterRepository)(bool , error)
func (_e *MockTreeClusterRepository_Expecter) Update(ctx interface{}, id interface{}, fn interface{}) *MockTreeClusterRepository_Update_Call {
	return &MockTreeClusterRepository_Update_Call{Call: _e.mock.On("Update", ctx, id, fn)}
}

func (_c *MockTreeClusterRepository_Update_Call) Run(run func(ctx context.Context, id int32, fn func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error))) *MockTreeClusterRepository_Update_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(int32), args[2].(func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)))
	})
	return _c
}

func (_c *MockTreeClusterRepository_Update_Call) Return(_a0 error) *MockTreeClusterRepository_Update_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *MockTreeClusterRepository_Update_Call) RunAndReturn(run func(context.Context, int32, func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)) error) *MockTreeClusterRepository_Update_Call {
	_c.Call.Return(run)
	return _c
}

// NewMockTreeClusterRepository creates a new instance of MockTreeClusterRepository. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewMockTreeClusterRepository(t interface {
	mock.TestingT
	Cleanup(func())
}) *MockTreeClusterRepository {
	mock := &MockTreeClusterRepository{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
