...
type TreeCountRepo interface {
	GetCount(ctx context.Context, query shared.TreeQuery) (int64, error)
}
...
type TreeClusterCountRepo interface {
	GetCount(ctx context.Context, query shared.TreeClusterQuery) (int64, error)
}
...
func (r *InfoRepository) GetAppInfo(ctx context.Context) (*shared.App, error) {
	return &shared.App{
		Version:     version,
...
func (r *InfoRepository) GetMapInfo(_ context.Context) (*shared.Map, error) {
	return &r.mapInfo, nil
}

func (r *InfoRepository) GetServerInfo(ctx context.Context) (*shared.Server, error) {
	log := logger.GetLogger(ctx)

	hostname, err := r.getHostname()
	if err != nil {
		log.Error("failed to get hostname from host", "error", err)
		return nil, shared.ErrHostnameNotFound
	}

	appURL, err := r.getAppURL()
	if err != nil {
		log.Error("failed to parse configured app url", "error", err, "app_url", r.cfg.Server.AppURL)
		return nil, shared.ErrCannotGetAppURL
	}
...
func (r *InfoRepository) GetServices(ctx context.Context) (*shared.Services, error) {
	services := r.getServices(ctx)
	return &services, nil
}

func (r *InfoRepository) GetStatistics(ctx context.Context) (*shared.DataStatistics, error) {
	stats := &shared.DataStatistics{}

	if r.treeRepo != nil {
		count, err := r.treeRepo.GetCount(ctx, shared.TreeQuery{})
		if err == nil {
	...
	if r.treeClusterRepo != nil {
		count, err := r.treeClusterRepo.GetCount(ctx, shared.TreeClusterQuery{})
		if err == nil {
...
func (r *InfoRepository) getServices(ctx context.Context) shared.Services {
	checker := newHealthChecker(r)
	services := checker.checkAll(ctx)
	return shared.Services{Items: services}
}

func (r *InfoRepository) getVersionInfo(ctx context.Context) shared.VersionInfo {
...
func getMapInfo(cfg *config.Config) (shared.Map, error) {
	if len(cfg.Map.Center) != 2 || len(cfg.Map.BBox) != 4 {
		return shared.Map{}, shared.ErrInvalidMapConfig
	}
...
func getIP(ctx context.Context) (net.IP, error) {
	...
	if err != nil {
		return nil, shared.ErrIPNotFound
	}
...
func getInterface(localIP net.IP) (string, error) {
	...
	if err != nil {
		return "", shared.ErrIFacesNotFound
	}

	for _, iface := range ifaces {
		address, err := iface.Addrs()
		if err != nil {
			return "", shared.ErrIFacesAddressNotFound
		}
