...
func ExecMigration(db *sql.DB) {
	...
	migrationPath := fmt.Sprintf("%s/internal/infrastructure/postgres/migrations/", rootDir)
...
func InsertSeed(db *sql.DB) {
	...
	rootDir := utils.RootDir()
	seedPath := fmt.Sprintf("%s/internal/infrastructure/postgres/seed/test", rootDir)
