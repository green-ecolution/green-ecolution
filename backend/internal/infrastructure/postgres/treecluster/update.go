...
func (r *TreeClusterRepository) Update(ctx context.Context, id int32, updateFn func(*shared.TreeCluster, shared.TreeClusterRepository) (bool, error)) error {
...
func (r *TreeClusterRepository) updateEntity(ctx context.Context, tc *shared.TreeCluster) error {
	log := logger.GetLogger(ctx)
	...
	if len(tc.Trees) > 0 {
		treeIDs := utils.Map(tc.Trees, func(t *shared.Tree) int32 {
			return t.ID
		})
