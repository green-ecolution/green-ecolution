...
func BindAndValidate[T any](c *fiber.Ctx) (*T, error) {
	...
	if err := c.BodyParser(&req); err != nil {
		return nil, errorhandler.HandleError(ports.NewError(ports.BadRequest, "invalid request body"))
	}
	if err := validate.Struct(&req); err != nil {
		return nil, errorhandler.HandleError(ports.NewError(ports.BadRequest, formatValidationError(err)))
	}
	return &req, nil
}
