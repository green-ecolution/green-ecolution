...
func Login(svc ports.AuthService) fiber.Handler {
...
func Logout(svc ports.AuthService) fiber.Handler {
	return func(c *fiber.Ctx) error {
		ctx := c.Context()
		...
		if err != nil {
			return errorhandler.HandleError(ports.NewError(ports.InternalError, errors.Wrap(err, "failed to logout").Error()))
		}
...
func RequestToken(svc ports.AuthService) fiber.Handler {
	return func(c *fiber.Ctx) error {
		ctx := c.Context()
		...
		if err != nil {
			return errorhandler.HandleError(ports.NewError(ports.InternalError, errors.Wrap(err, "failed to get token").Error()))
		}
...
// @Router			/v1/user [post]
// @Security		Keycloak
func Register(svc ports.AuthService) fiber.Handler {
	return func(c *fiber.Ctx) error {
		ctx := c.Context()
...
// @Security		Keycloak
func GetUsersByRole(svc ports.AuthService) fiber.Handler {
	return func(c *fiber.Ctx) error {
		ctx := c.Context()
		...
		if err != nil {
			return errorhandler.HandleError(ports.NewError(ports.InternalError, errors.Wrap(err, "failed to get users by role").Error()))
		}
...
func RefreshToken(svc ports.AuthService) fiber.Handler {
	return func(c *fiber.Ctx) error {
		ctx := c.Context()
