...
func Test_JWTMiddleware_TokenValidation(t *testing.T) {
	t.Run("should success on introspect token", func(t *testing.T) {
		...
		// when
		authSvc.EXPECT().RetrospectToken(mock.Anything, mock.Anything).Return(&auth.IntroSpectTokenResult{Active: utils.P(true)}, nil)
		got, err := NewJWTMiddleware(cfg, authSvc)
		assert.NoError(t, err)
		app := fiber.New()
		app.Use(got)
	...
	t.Run("should return code 401 on inactive token", func(t *testing.T) {
		...
		// when
		authSvc.EXPECT().RetrospectToken(mock.Anything, mock.Anything).Return(&auth.IntroSpectTokenResult{Active: utils.P(false)}, nil)
		got, err := NewJWTMiddleware(cfg, authSvc)
		assert.NoError(t, err)
		app := fiber.New()
		app.Use(got)
