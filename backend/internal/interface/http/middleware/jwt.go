...
import (
	...
	"github.com/green-ecolution/green-ecolution/backend/internal/interface/http/middleware/jwks"
	"github.com/green-ecolution/green-ecolution/backend/internal/interface/http/wrapper"
...
func NewJWTMiddleware(cfg *config.IdentityAuthConfig, svc ports.AuthService) (fiber.Handler, error) {
	if !cfg.Enable {
		return func(c *fiber.Ctx) error {
	...
	return contribJwt.New(contribJwt.Config{
		...
		ErrorHandler: func(c *fiber.Ctx, err error) error {
			...
			err = ports.NewError(ports.Unauthorized, err.Error())
...
func successHandler(c *fiber.Ctx, svc ports.AuthService) error {
