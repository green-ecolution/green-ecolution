...
INSERT INTO tree_clusters (id, name, watering_status, moisture_level, region_id, address, description, soil_condition, latitude, longitude, geometry) VALUES
  ('019e3fcb-2a53-7fbf-948d-36836e4f42d1'::uuid, 'Solitüde Strand', 'just watered', 0.85, (SELECT id FROM regions WHERE name = 'Mürwik'), 'Solitüde Strand', 'Alle Bäume am Strand', 'Ss', 54.82128536520703, 9.488152515892045, ST_SetSRID(ST_MakePoint(9.488152515892045, 54.82128536520703), 4326)),
  ('019e3fcb-2a53-7619-aa57-c9b2c9a4aa29'::uuid, 'Sankt-Jürgen-Platz', 'moderate', 0.5, (SELECT id FROM regions WHERE name = 'Mürwik'), 'Ulmenstraße', 'Bäume beim Sankt-Jürgen-Platz', 'Uu', 54.78805731048199, 9.44400186680097, ST_SetSRID(ST_MakePoint(9.44400186680097, 54.78805731048199), 4326)),
  ('019e3fcb-2a53-7bc9-bc1c-a51e04ef632e'::uuid, 'Flensburger Stadion', 'unknown', 0.7, (SELECT id FROM regions WHERE name = 'Mürwik'), 'Flensburger Stadion', 'Alle Bäume in der Gegend des Stadions in Mürwik', 'Uu', 54.802163, 9.446398, ST_SetSRID(ST_MakePoint(9.446398, 54.802163), 4326)),
  ('019e3fcb-2a53-7493-a7a1-b194faa409d7'::uuid, 'Campus Hochschule', 'bad', 0.1, (SELECT id FROM regions WHERE name = 'Sandberg'), 'Thomas-Finke Straße', 'Gruppe ist besonders anfällig', 'Uu', 54.77576059694547, 9.450720736264868, ST_SetSRID(ST_MakePoint(9.450720736264868, 54.77576059694547), 4326)),
  ('019e3fcb-2a53-713d-8cae-4a52d3fd2967'::uuid, 'Mathildenstraße', 'bad', 0.4, (SELECT id FROM regions WHERE name = 'Friesischer Berg'), 'Mathildenstraße', 'Sehr enge Straße und dadurch schlecht zu bewässern.', 'Uu', 54.78219253876479, 9.423978982828825, ST_SetSRID(ST_MakePoint(9.423978982828825, 54.78219253876479), 4326)),
  ('019e3fcb-2a53-7706-a04c-50d6c926c7f8'::uuid, 'Nordstadt', 'good', 0.8, (SELECT id FROM regions WHERE name = 'Nordstadt'), 'Apenrader Straße', 'Guter Baumbestand mit großen Kronen.', 'Ss', 54.807162, 9.423138, ST_SetSRID(ST_MakePoint(9.423138, 54.807162), 4326)),
  ('019e3fcb-2a53-7963-8228-705689f884b9'::uuid, 'TSB Neustadt', 'just watered', 0.9, (SELECT id FROM regions WHERE name = 'Nordstadt'), 'Ecknerstraße', 'Kleiner Baumbestand.', 'Ss', 54.797162, 9.419620, ST_SetSRID(ST_MakePoint(9.419620, 54.797162), 4326)),
  ('019e3fcb-2a53-7be5-9a56-e2260bb1c9fb'::uuid, 'Seniorenanlage Valentinerhof', 'bad', 0.15, (SELECT id FROM regions WHERE name = 'Nordstadt'), 'Auf dem Geländer der Seniorenanlage', 'Sehr viel versiegelter Boden.', 'Ss', 54.76994251235151, 9.441111747447234, ST_SetSRID(ST_MakePoint(9.441111747447234, 54.76994251235151), 4326)),
  ('019e3fcb-2a53-7433-8d57-6258f9688c82'::uuid, 'Peelwatt', 'unknown', 0.3, (SELECT id FROM regions WHERE name = 'Nordstadt'), 'Peelwatt halt', 'Sehr viel versiegelter Boden.', 'Ss', 54.76671656688957, 9.456136954289867, ST_SetSRID(ST_MakePoint(9.456136954289867, 54.76671656688957), 4326)),
  ('019e3fcb-2a53-7fc1-bc32-9c8885df5027'::uuid, 'Lautrupsbach', 'moderate', 0.45, (SELECT id FROM regions WHERE name = 'Nordstadt'), 'An der Nordstraße', 'Sehr viel versiegelter Boden.', 'Ss', 54.79265065021804, 9.454269041383837, ST_SetSRID(ST_MakePoint(9.454269041383837, 54.76671656688957), 4326));
